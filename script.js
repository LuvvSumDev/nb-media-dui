...
function resolveUrl(url) {
    if (url.startsWith('http://') || url.startsWith('https://')) {
        ...
        return 'http://' + currentServerEndpoint + '/nb-media/media/' + url;
    }
}
...
function initPlayer(id, handle, options) {
    ...
    new MediaElement(id, {
        ...
        success: function (media, domNode) {
            ...
            media.nbMedia = {};
            media.nbMedia.initialized = false;
            media.nbMedia.attenuationFactor = options.attenuation.diffRoom;
            media.nbMedia.volumeFactor = options.diffRoomVolume;
            ...
            media.addEventListener('canplay', () => {
                ...
                media.nbMedia.initialized = true;

                media.play();
            });

            media.addEventListener('playing', () => {
                if (options.filter && !media.nbMedia.filterAdded) {
                    if (isRDR) {
                        applyPhonographFilter(media);
                    } else {
                        applyRadioFilter(media);
                    }
                    media.nbMedia.filterAdded = true;
                }

                if (
                    options.visualization &&
                    !media.nbMedia.visualizationAdded
                ) {
                    createAudioVisualization(media, options.visualization);
                    media.nbMedia.visualizationAdded = true;
                }
            });
...
function setAttenuationFactor(player, target) {
    if (player.nbMedia.attenuationFactor > target) {
        player.nbMedia.attenuationFactor -= 0.1;
    } else {
        player.nbMedia.attenuationFactor += 0.1;
    }
}
...
function setVolumeFactor(player, target) {
    if (player.nbMedia.volumeFactor > target) {
        player.nbMedia.volumeFactor -= 0.01;
    } else {
        player.nbMedia.volumeFactor += 0.01;
    }
}
        ...
        if (
            ...
            if (player.readyState > 0) {
                ...
                if (data.options.muted || data.volume == 0) {
                    ...
                    volume =
                        ((100 -
                            data.distance * player.nbMedia.attenuationFactor) /
                            100) *
                        player.nbMedia.volumeFactor *
                        (data.volume / 100);
                }
                ...
                if (data.options.duration) {
                    ...
                    if (
                        Math.abs(currentTime - player.currentTime) >
                        maxTimeDifference
                    ) {

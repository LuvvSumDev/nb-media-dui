var Wave = (function () {
    ...
    function fromElement(element_id, canvas_id, options) {
        ...
        function run() {
            ...
            this.activeElements[element_id] =
                this.activeElements[element_id] || {};
    ...
    var drawOrbs = (functionContext) => {
        ...
        points.start.forEach((start, i) => {
            ...
            helper.drawCircle(start, h * 0.01, {
    ...
    var drawCubes = (functionContext) => {
        ...
        points.start.forEach((start, i) => {
            ...
            for (let j = 1; j <= squareCount; j++) {
                let origin = [
                    start[0],
                    start[1] - squareSize * j - spacing * j,
                ];
    ...
    var origami_1 = createCommonjsModule(function (module, exports) {
        ...
        (function (window) {
            ...
            function normalizeStyle(style) {
                ...
                if (style.border) {
                    ...
                    border = border.concat(
                        style.border.match(/[0-9]*\.?[0-9]px?/i)
                    );
                    ...
                    // 1 - Style
                    border = border.concat(
                        borderString.match(/solid|dashed|dotted/i)
                    );
                    borderString = borderString.replace(
                        /solid|dashed|dotted/i,
                        ''
                    );
                    ...
                    if (hasOwn.call(b, prop)) {
                        ...
                        if (prop !== 'constructor' || a !== commonjsGlobal) {
                            if (b[prop] === undefined) {
                                ...
                                !(undefOnly && typeof a[prop] !== 'undefined')
                            ) {
                                a[prop] = b[prop];
                            }
                        }
            ...
            function styleRuleValueFrom(selector, documentStyleRules) {
                for (var j = 0; j < documentStyleRules.length; j++) {
                    if (
                        ...
                        documentStyleRules[j].selectorText.toLowerCase() ===
                            selector
                    ) {
            ...
            Screen.prototype.composition = function (params) {
                this.paper.ctx.globalCompositeOperation =
                    params.globalComposite;
            };
            ...
            Origami.image = function (image, x, y, width, height) {
                ...
                var reference = self.paper.queue.length - 1,
                    currentQueue =
                        config.contexts[this.paper.index].queue[reference];

                image.addEventListener('load', function () {
                    if (!currentQueue) return false;
                    currentQueue.params.width =
                        item.width || image.naturalWidth;
                    currentQueue.params.height =
            ...
            function ChartLine(config) {
                ...
                var line = getBorderStyleObject(
                    config.line || '1px solid #000'
                );
                var lineVariance = 2;
            ...
            Origami.startRender = function (fn) {
                ...
                self.draw(function () {
                    self.paper.frame = window.requestAnimationFrame(
                        fn.bind(this)
                    );
                });
            };
        ...
        mutateData(data, type, extra = null) {
            ...
            if (type === 'shrink') {
                ...
                for (let i = 1; i <= extra; i++) {
                    let arraySection = data.slice(
                        i * splitAt,
                        i * splitAt + splitAt
                    );
                    let middle =
                        arraySection[Math.floor(arraySection.length / 2)];
                    rtn.push(middle);
                }
        ...
        getPoints(
            ...
            if (shape === 'circle') {
                ...
                for (let i = 1; i <= pointCount; i++) {
                    ...
                    let x =
                        originX +
                        (radius - pointOffset) * Math.cos(currentRadian);
                    let y =
                        originY +
                        (radius - pointOffset) * Math.sin(currentRadian);
                    let point1 = this.__rotatePoint__(
                        [x, y],
                        [originX, originY],
                        rotate
                    );
                    ...
                    x =
                        originX +
                        (radius - pointOffset + currentEndPoint) *
                            Math.cos(currentRadian);
                    y =
                        originY +
                        (radius - pointOffset + currentEndPoint) *
                            Math.sin(currentRadian);
                    let point2 = this.__rotatePoint__(
                        [x, y],
                        [originX, originY],
                        rotate
                    );
            ...
            if (shape === 'line') {
                ...
                for (let i = 0; i <= pointCount; i++) {
                    ...
                    let endingPoint = this.__rotatePoint__(
                        [
                            originX + i * increment,
                            originY + endPoints[i] - pointOffset,
                        ],
                        [originX, originY],
                        degree
                    );
